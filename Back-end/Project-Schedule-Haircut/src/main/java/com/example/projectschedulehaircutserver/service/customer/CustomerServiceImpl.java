...
public class CustomerServiceImpl implements CustomerService{
    ...
    @Override
    public Set<CustomerManagementResponse> getAccountManagement() throws CustomerException {
        try {
            Set<CustomerManagementResponse> result = customerRepo.getAccountManagement();
            if(result.isEmpty()) {
                throw new NoSuchElementException("Danh sách khách hàng trống");
            }
            return result;
        } catch (DataAccessException ex) {
            throw new CustomerException("Lỗi truy vấn dữ liệu");
        }
    }
